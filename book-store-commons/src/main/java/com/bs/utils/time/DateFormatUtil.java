package com.bs.utils.time;

import java.text.ParseException;
import java.util.Date;

import com.bs.annotation.NotNull;
...
public class DateFormatUtil {
     ...
     * @return
     */
	public static String getDayBeforeOrAfter(Date date) {
        long now = ClockUtil.currentTimeMillis();
        long span = now - date.getTime();
        if (span < 0) {
            long s = Math.abs(span);
            if (s < DateUtil.MILLIS_PER_MINUTE) {
                return String.format("%d秒后", s / DateUtil.MILLIS_PER_SECOND);
                ...
                return String.format("%d天后", s / DateUtil.MILLIS_PER_DAY);
            }
        } else {
            if (span < DateUtil.MILLIS_PER_MINUTE) {
                ...
                return String.format("%d天前", span / DateUtil.MILLIS_PER_DAY);
            }
        }
    }
}
